...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val classification = points.groupBy(p => findClosest(p, means))
    classification ++ (means.filter(m => !classification.contains(m))).map(m => (m,Nil)).toMap
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    oldMeans.zip(newMeans).map((mm : (Point, Point)) => (mm._1.squareDistance(mm._2) <= eta)).fold(true)((b1: Boolean, b2: Boolean) => b1 && b2)
  }
